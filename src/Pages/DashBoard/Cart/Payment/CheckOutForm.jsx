...
const CheckOutForm = () => {
    ...
    useEffect(() => {
        if (totalPrice > 0) {
            axiosSecure.post('/create-payment-intent', { average_cost: totalPrice })
                .then(res => {
...
const payment = {
                   email: user.email,
                   average_cost: totalPrice,
                   transactionId: paymentIntent.id,
                   date: new Date(), // utc date convert. use moment js to 
                   cartIds: cart.map(list => list._id),     
                   touristIds: cart.map(list=> list.touristId),
                   status: 'pending'
               }
